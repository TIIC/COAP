...
#
def averageStats(para, endSlice):
	# UMEAN
	skippedHeader = 28
	...
	print 'UMEAN:'
	extractRaw(para, endSlice, skippedHeader, skippedFooter, resultFolder)
...
#
def extractRaw(para, endSlice, skippedHeader, skippedFooter, resultFolder):
	if para['dataType'] == 'rel':
		startSlice = 0
	else: # for case 'rt'
		startSlice = int(endSlice * (1 - para['slicesToTest']))
	for den in para['density']:
		for timeslice in xrange(startSlice, endSlice):
			...
			if timeslice == startSlice:
				result = np.zeros((data.shape[0], data.shape[1], endSlice - startSlice))
			result[:, :, timeslice - startSlice] = data
